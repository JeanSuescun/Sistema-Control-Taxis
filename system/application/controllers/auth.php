...
class Auth extends Controller
{
	...
	function username_check($username)
	{
		...
		if ( ! $result)
		{
			...
			$this->form_validation->set_message('username_check', 'Nombre de usuario ya existe. Por favor, elija otro nombre de usuario.');
		}
	...
	function email_check($email)
	{
		...
		if ( ! $result)
		{
			...
			$this->form_validation->set_message('email_check', 'El correo electr&oacute;nico ya es utilizado por otro usuario. Por favor, elija otra direcci&oacute;n de correo electr&oacute;nico.');
		}
	...
	function captcha_check($code)
	{
		...
		if ($this->dx_auth->is_captcha_expired())
		{
			...
			$this->form_validation->set_message('captcha_check', 'Su c&oacute;digo de confirmaci&oacute;n ha expirado. Por favor, int&eacute;ntelo de nuevo.');
			$result = FALSE;
		}
		elseif ( ! $this->dx_auth->is_captcha_match($code))
		{
			...
			$this->form_validation->set_message('captcha_check', 'Su c&oacute;digo de confirmaci&oacute;n no coincide con el de la imagen. Int&eacute;ntelo de nuevo.');			
			$result = FALSE;
		}
	...
	function recaptcha_check()
	{
		...
		if ( ! $result)
		{
			...
			$this->form_validation->set_message('recaptcha_check', 'Su c&oacute;digo de confirmaci&oacute;n no coincide con el de la imagen. Int&eacute;ntelo de nuevo.');
		}
	...
	function login()
	{
        ...
        else
        {
			...
			$val->set_rules('username', 'Nombre de Usuario', 'trim|required|xss_clean');
			$val->set_rules('password', 'Contrase&ntilde;a', 'trim|required|xss_clean');
			$val->set_rules('remember', 'Remember me', 'integer');

			// Set captcha rules if login attempts exceed max attempts in config
			if ($this->dx_auth->is_max_login_attempts_exceeded())
			{
				$val->set_rules('captcha', 'C&oacute;digo de Confirmaci&oacute;n', 'trim|required|xss_clean|callback_captcha_check');
			}
		...
		else
		{
			...
			$data['auth_message'] = 'Usted ya inici&oacute; su sesi&oacute;n.';
	...
	function logout()
	{
		...
		$data['auth_message'] = 'Usted ha cerrado su sesi&oacute;n.';
	...
	{		
		...
		{	
			...
			// Set form validation rules			
			$val->set_rules('username', 'Nombre de Usuario', 'trim|required|xss_clean|min_length['.$this->min_username.']|max_length['.$this->max_username.']|callback_username_check|alpha_dash');
			$val->set_rules('password', 'Contrase&ntilde;a', 'trim|required|xss_clean|min_length['.$this->min_password.']|max_length['.$this->max_password.']|matches[confirm_password]');
			$val->set_rules('confirm_password', 'Confirmar contrase&ntilde;a', 'trim|required|xss_clean');
			$val->set_rules('email', 'Correo electr&oacute;nico', 'trim|required|xss_clean|valid_email|callback_email_check');
			
			if ($this->dx_auth->captcha_registration)
			{
				$val->set_rules('captcha', 'C&oacute;digo de confirmaci&oacute;n', 'trim|xss_clean|required|callback_captcha_check');
			}
			...
			{	
				...
				if ($this->dx_auth->email_activation)
				{
					...
					$data['auth_message'] = 'Ud se ha registrado con &eacute;xito. Revise su correo para activar su cuenta.';
				}
				else
				{					
					//$data['auth_message'] = 'You have successfully registered. '.anchor(site_url($this->dx_auth->login_uri), 'Login');
					$data['auth_message'] = 'Ud se ha registrado con &eacute;xito '.anchor(site_url($this->dx_auth->login_uri), 'Entrar');
				}
		...
		elseif ( ! $this->dx_auth->allow_registration)
		{
			...
			$data['auth_message'] = 'El registro ha sido desactivado.';
			$this->load->view($this->dx_auth->register_disabled_view, $data);
		}
		else
		{
			//$data['auth_message'] = 'You have to logout first, before registering.';
			$data['auth_message'] = 'Hay que cerrar la sesi&oacute;n en primer lugar, antes de registrarse.';
			$this->load->view($this->dx_auth->logged_in_view, $data);
		}
	}
	...
	function register_recaptcha()
	{
		...
		{	
			...
			if ($this->dx_auth->captcha_registration)
			{
				...
				$val->set_rules('recaptcha_response_field', 'C&oacute;digo de confirmaci&oacute;n', 'trim|xss_clean|required|callback_recaptcha_check');
			}
			...
			{	
				...
				{
					//$data['auth_message'] = 'You have successfully registered. Check your email address to activate your account.';
					$data['auth_message'] = 'Ha registrado. Compruebe su direcci&oacute;n de correo electr&oacute;nico para activar tu cuenta.';
				}
				else
				{					
					//$data['auth_message'] = 'You have successfully registered. '.anchor(site_url($this->dx_auth->login_uri), 'Login');
					$data['auth_message'] = 'Ha registrado. '.anchor(site_url($this->dx_auth->login_uri), 'Entrar');
				}
	...
	function activate()
	{
		...
		if ($this->dx_auth->activate($username, $key)) 
		{
			...
			$data['auth_message'] = 'Su cuenta ha sido activada con &eacute;xito. '.anchor(site_url($this->dx_auth->login_uri), 'Entrar');
		...
		else
		{
			...
			$data['auth_message'] = 'El c&oacute;digo de activaci&oacute;n que ha introducido es incorrecto. Por favor revise su correo electr&oacute;nico de nuevo.';
	...
	function forgot_password()
	{
		...
		// Set form validation rules
		$val->set_rules('login', 'Usuario o direcci&oacute;n de correo', 'trim|required|xss_clean');
		...
		if ($val->run() AND $this->dx_auth->forgot_password($val->set_value('login')))
		{
			...
			$data['auth_message'] = 'Un correo electr&oacute;nico ha sido enviado a su direcci&oacute;n, con instrucciones para activar su nueva contrase&ntilde;a.';
	...
	function reset_password()
	{
		...
		if ($this->dx_auth->reset_password($username, $key))
		{
			...
			$data['auth_message'] = 'Se ha cambiado su contrase&ntilde;a exitosamente, '.anchor(site_url($this->dx_auth->login_uri), 'Entrar');
		...
		else
		{
			...
			$data['auth_message'] = 'Su nombre de usuario y clave son correctos. Por favor revise su correo electr&oacute;nico de nuevo y siga las instrucciones.';
